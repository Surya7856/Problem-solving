...
class Solution
{
    private static boolean isDead(Node node, int min, int max) {
        if (node == null) {
            return false;
        }
        if (min == max) {
        ...
        boolean leftDeadEnd = isDead(node.left, min, node.data - 1);
        boolean rightDeadEnd = isDead(node.right, node.data + 1, max);
