...
class Solution
{
    static void visit(Node root,HashSet<Integer> h){
        if(root==null){
            return;
        }
        h.add(root.data);
    ...
    static int check(Node root,HashSet<Integer> h,int x){
        if(root==null){
            return 0;
        }
        if(h.contains(x-root.data)){
            return 1+check(root.left,h,x)+check(root.right,h,x);
        }
        else
            return check(root.left,h,x)+check(root.right,h,x);
